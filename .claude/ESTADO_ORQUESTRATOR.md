...
| 12  | `GAP-012` | Testes      | P4     | `test/backend-test/monitors/test-tcp.js:97` ("check() rejects when TLS certificate is expired or invalid") depende de rede externa real (`expired.badssl.com:443`) — achado na 1ª rodada real de CI (Actions nunca tinha rodado neste repo): falhou só em `windows-latest`+Node 24 (`Error: Connection failed` genérico em vez do erro TLS esperado), passou em macOS/ubuntu/ubuntu-arm/armv7/windows-node20. Sintoma de flakiness de rede/runner, não bug em `tcp.js` (não tocado por nenhuma mudança recente). Candidato a mockar o host em vez de depender de serviço externo. | T2   | 🟡 queued                                                                                                                                                                                            |
