...
- id: TASK-R4
  ...
  risco: T3
  depends_on: [TASK-R3]
  status: in_progress # commits 39dbe802 (R7) + cb41b96a (R8+attachActor+rooms), feature/rbac-multitenant, NÃO pushed p/ produção (só no origin do fork). Concluído até aqui: (1) R7 -- findOrCreateMirroredMonitor + addRemoteInstance agora setam team_id (bug irmão achado: addRemoteInstance nunca setava team_id, o que teria anulado o fix em findOrCreateMirroredMonitor); getRemoteInstanceList migrado p/ scopeFilter. (2) R8 -- isMonitorPublic exige status_page.team_id = monitor.team_id (fecha badge-leak), seguro p/ qualquer install de hoje (só 1 team). (3) attachActor/requireSuperadmin em server/auth.js (resolve req.actor espelhando os 3 modos do apiAuth: disableAuth/API-key/basic-auth) + /metrics gateado a superadmin. (4) server/security/rooms.js (roomFor helper p/ o refactor de rooms). 24 testes novos, tudo lint limpo. **Restante da P4 (maior parte)**: retrofit de ~11 sites de socket.join/io.to espalhados (server.js, uptime-kuma-server.js, client.js, model/monitor.js, model/status_page.js, api-router.js /push, federation-router.js, cloudflared-socket-handler.js) p/ usar roomFor(); sync de boot do rbacEnforced (setEnforcementEnabled a partir de Settings.get); guard de "last superadmin"; fixture E2E 2-teams provando o flip de ponta a ponta. **Nota de ambiente**: sessão sofreu contenção de recursos real (processos node de OUTRAS atividades concorrentes na mesma máquina compartilhada) causando hangs em testes combinados pesados (múltiplos servidores HTTP no mesmo processo) -- mitigado rodando suítes pesadas isoladas em vez de combinadas.
