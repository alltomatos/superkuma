exports.up = function (knex) {
    return knex.schema.createTable("stat_monthly", function (table) {
        table.increments("id");
        table.comment("This table contains the monthly aggregate statistics for each monitor (long-term retention tier)");
        table
            .integer("monitor_id")
            .unsigned()
            .notNullable()
        ...
        table.float("ping_min", 20, 2).notNullable().defaultTo(0).comment("Minimum ping during this period in milliseconds");
        table.float("ping_max", 20, 2).notNullable().defaultTo(0).comment("Maximum ping during this period in milliseconds");
