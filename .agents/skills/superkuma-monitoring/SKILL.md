...
1. `get_info` — confirm the connection (version, monitor count) and check its `permissions`
   field (`canCreateMonitors`/`canUpdateMonitors`/`canDeleteMonitors`) to see what this API key's
   **role** actually permits — there's no separate server-side write/delete toggle, the role
   (Owner/Admin/Editor/Viewer, set when the key was created) is the only control
   ([ADR-0018](../../../docs/adr/0018-mcp-rbac-only-no-env-var-gates.md)). A Viewer-role key gets
   a permission error on any write; ask for the key to be reissued with Editor+ if that happens.
2. `list_monitors` — see what already exists; never create a duplicate.
3. `delete_*` tools still require `confirm: true` per call regardless of role — a dry-run
   describes what would happen until you pass it.
...
6. **Verify** — `list_monitors`, then `get_monitor_beats` on a few monitors to confirm they are
   actually checking.
7. **Deep metrics (optional)** — for CPU/RAM/disk I/O and SQL Server internals beyond up/down,
   pick the path that matches what the target already speaks:
   - Exposes a Prometheus `/metrics` endpoint, or you're installing an exporter yourself
     (`node_exporter`, `windows_exporter` — installer script in
     [assets/windows/install-windows_exporter.ps1](assets/windows/install-windows_exporter.ps1) —
     the SQL Server exporter) — set up Prometheus
     ([references/prometheus-exporters.md](references/prometheus-exporters.md), stack in
     [assets/prometheus-stack/](assets/prometheus-stack/)) and add `prometheus`-type monitors.
   - Already ships Telegraf natively and has no Prometheus endpoint (a **pfSense** firewall with
     the Telegraf package — CPU/load/memory/packet-loss/gateway-RTT — or other managed
     switches/routers) — feed it into InfluxDB instead. SuperKuma's own `compose.yaml` already
     bundles an InfluxDB v1 container for exactly this
     ([references/pfsense-telegraf.md](references/pfsense-telegraf.md)) — no separate stack to
     stand up, no Prometheus needed. Add `influxdb`-type monitors pointed at it.

   Both monitor types share the same
   [inverted-threshold gotcha](references/monitor-mapping.md#deep-host-metrics-via-prometheus-cpuramdisk-io-sql-server) —
   `conditionOperator` describes the healthy side, UP while TRUE. Set `metricUnit` (`%`, `GB`, …)
   on these — and on any numeric `snmp` / `json-query` monitor — so the detail page shows a value
   gauge + trend chart (`%` gives a fixed 0-100 scale); it's the unit that turns a threshold check
   into a readable metric dial.
