...
Every tool is always registered — there is no server-side env var that turns write/delete tools
on or off (see [ADR-0018](../../../../docs/adr/0018-mcp-rbac-only-no-env-var-gates.md)). What a
call can actually do is decided entirely by the API key's **role** (Owner/Admin/Editor/Viewer),
the same RBAC every dashboard user goes through — a Viewer-role key gets a permission error on
any write, no env var involved. Check `get_info`'s `permissions` field
(`canCreateMonitors`/`canUpdateMonitors`/`canDeleteMonitors`) to see what this specific
connection can do before attempting a write. `delete_*` is still a dry-run unless
`confirm: true` per call, regardless of role.
