...
services:
  superkuma:
    ...
    depends_on: [mariadb]
    environment:
      # What an API key can do (read/create/update/delete) is decided entirely by its role,
      # chosen when the key is created (Settings > API Keys) -- no separate write/delete
      # env var to also set (ADR-0018).
...
## 6. Operational gotchas after the instance is live

### The SuperKuma host/VM's own IP should be a DHCP static mapping, not a static IP inside the guest

Don't set a static IP inside the VM's own network config (netplan/cloud-init `ipconfig0`) if the
network's DHCP server (pfSense, a router) supports **static mappings** (a.k.a. DHCP reservations,
bind IP to MAC). A reservation on the DHCP server side means the VM keeps behaving like a normal
DHCP client — no netplan drift to maintain, no risk of it double-issuing an IP that also gets
statically assigned inside the guest — while still always getting the same IP. Prefer this over
an in-guest static IP whenever the option exists.

### Changing that IP means updating every monitor/config that points at the old one

If the host's IP does change (DHCP lease change, a reservation gets edited, the VM gets
recreated), everything that hardcoded the old IP breaks and needs updating:

- Any monitor pointing `hostname`/`url` at the instance's own IP (self-monitoring the SuperKuma
  process, MariaDB, Prometheus, InfluxDB — see section 2's compose shapes).
- Every remote MCP client config pointing `http://<old-ip>:3001/mcp` (e.g. a `~/.claude.json`
  `mcpServers` entry) — these live outside SuperKuma's own data and won't fix themselves; each
  client needs its config edited and (for most MCP clients, since server connections are
  typically established at startup) the client session restarted to pick up the change.
- SuperKuma **caches monitor definitions in memory** — editing the underlying DB row directly
  (rather than through the API/dashboard) does not take effect until the container restarts:
  ```bash
  docker exec <mariadb-container> mariadb -u"$MARIADB_USER" -p"$MARIADB_PASSWORD" superkuma -e \
    "UPDATE monitor SET hostname='<new-ip>' WHERE hostname='<old-ip>';
     UPDATE monitor SET url=REPLACE(url,'<old-ip>','<new-ip>') WHERE url LIKE '%<old-ip>%';"
  docker restart superkuma
  ```
  Prefer the MCP `update_monitor` tool / dashboard edit over a raw SQL `UPDATE` when the MCP
  connection is reachable — this direct-SQL path is the fallback for exactly the case where it
  isn't (the instance's own IP just moved, so the MCP client pointed at the old address can't
  reach it either).

### A single monitor's scheduler can get permanently stuck with zero heartbeats

Rare but observed in the field: a monitor created normally (`active: true`, valid config) simply
never produces a heartbeat — `get_monitor_beats` returns an empty array indefinitely, even hours
later, while every other monitor created in the same batch works fine. `pause_monitor` +
`resume_monitor` and `update_monitor` (even just re-setting `interval` to force a config change)
do **not** unstick it — the in-memory scheduler job for that specific monitor is gone, not
misconfigured. The only fix found: `docker restart superkuma` — this forces a full reload of
every monitor's scheduled job from the DB. Verify the fix by checking that monitor's beats a
minute or two after the restart.
