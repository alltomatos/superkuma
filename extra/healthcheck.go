...
func main() {
	...
	sslKey := os.Getenv("SUPERKUMA_SSL_KEY")
	if len(sslKey) == 0 {
		sslKey = os.Getenv("SSL_KEY")
	}

	sslCert := os.Getenv("SUPERKUMA_SSL_CERT")
	if len(sslCert) == 0 {
		sslCert = os.Getenv("SSL_CERT")
	}

	hostname := os.Getenv("SUPERKUMA_HOST")
	...
	port := ""
	// SUPERKUMA_PORT is override by K8S unexpectedly,
	if !isK8s {
		port = os.Getenv("SUPERKUMA_PORT")
	}
	if len(port) == 0 {
		port = os.Getenv("PORT")
	}
